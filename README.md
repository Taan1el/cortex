...
Cortex is a small Obsidian side panel for working with notes using a local AI model.
...
```text
<your-vault>/.obsidian/plugins/cortex/
```
...
Cortex works best with Ollama running locally.
...
For note edits, Cortex shows a permission card before replacing the file. Pick **Apply edit** or **Keep original**.
...
Cortex can import ChatGPT exports:
...
If Cortex says Ollama is not available:
...
- check the folder name is exactly `cortex`
- check `manifest.json` is in that folder
- reload Obsidian

If note edits feel risky, leave write approval on. Cortex will ask before replacing the current note.
